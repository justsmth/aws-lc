  ...
  while (reloc_block_ptr + sizeof(IMAGE_BASE_RELOCATION) <= reloc_end) {
    ...
    for (size_t i = 0; i < entry_count; i++) {
      ...
      if (type == IMAGE_REL_BASED_DIR64) {
        reloc_width = sizeof(uint64_t);
      }
#else
      if (type == IMAGE_REL_BASED_HIGHLOW) {
        reloc_width = sizeof(uint32_t);
      }
#endif
      if (reloc_width == 0) {
        if (region_start <= reloc_addr && reloc_addr < region_end) {
          return 0;
        }
        continue;
      }

      if (reloc_addr < region_start) {
        if (reloc_width > (size_t)(region_start - reloc_addr)) {
          return 0;
        }
        continue;
      }
      if (reloc_addr >= region_end) {
        continue;
      }
      if (reloc_width > (size_t)(region_end - reloc_addr)) {
        return 0;
      }
      ...
      const uint32_t value =
          CRYPTO_load_u32_le(reloc_ptr) - (uint32_t)load_delta;
      CRYPTO_store_u32_le(adjusted, value);
#endif
      if (!HMAC_Update(hmac_ctx, adjusted, reloc_width)) {
        return 0;
      }
      cursor = reloc_ptr + reloc_width;
    }
